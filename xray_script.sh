...
function _xrayr_config(){
    curl -H "Authorization: Bearer "${OAUTHTOKEN}"" -Lo /usr/local/etc/xray/config.json "$xrayr_config"
    curl -H "Authorization: Bearer "${OAUTHTOKEN}"" -Lo /etc/XrayR/cert/nanodesu.key "$key"
    curl -H "Authorization: Bearer "${OAUTHTOKEN}"" -Lo /etc/XrayR/cert/Certificate.crt "$cer"
}
#环境
function _os() {
  local os=""
...
OAUTHTOKEN='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'
}
